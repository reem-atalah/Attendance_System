    ...
    def train(self, training):
        ...
        for ex in training:
            if ex[1] == 1:
                ...
                neg.append(ex)
        clf_pre = ViolaJones(T=10)
        X,y,X_len,training_set_integ,weights,Y_len,features=clf_pre.pre_train(pos+neg, len(pos), len(neg))
        for feature_num in self.layers:
            if len(neg) == 0:
                print("Stopping early. FPR = 0")
                break
            clf = ViolaJones(T=feature_num)
            clf.train(X,y,X_len,training_set_integ,weights,Y_len,features)
